...
### Distribution — two builds from one codebase (Mac App Store + direct)

Goal: a sandboxed Mac App Store edition alongside the current unsandboxed edition
distributed from GitHub/website, built from one codebase behind a compilation flag.

Evidence base: Apple requires App Sandbox for Mac App Store distribution ("To distribute
a macOS app through the Mac App Store, you must enable the App Sandbox capability"), and
`com.apple.security.files.all` is deprecated (macOS 10.7–10.11), so there is no blanket
file-access entitlement. However Powerbox grants a user-selected folder recursively, and
security-scoped bookmarks persist that grant across launches — so file management itself
survives sandboxing. Commander One ships exactly this two-edition split; its MAS build
keeps dual panes, unlimited tabs, regex search, full ZIP support and previews, and loses
only process control, terminal/command line, iOS mounting, root escalation, F-key
override and drive eject. DuPane's restricted surface maps almost one-to-one onto that.

#### Phase 0 — prerequisites

- **Confirm the distribution decision before any sandboxing work** — the direct edition needs no architectural change; the MAS edition needs Phase 2–3. Recommended order is direct first, MAS as a follow-on, but confirm before starting Phase 2.
- **Apple Developer Program enrolment (99 USD per membership year)** — required for *both* editions: Developer ID signing and notarisation for direct distribution, and App Store Connect for MAS. One enrolment covers both.

#### Phase 1 — direct edition (ship this first; no code changes needed)

- **Developer ID signing** — `ENABLE_HARDENED_RUNTIME: YES` is already set in `project.yml`, which is the main prerequisite. Add `DEVELOPMENT_TEAM` and Developer ID signing settings.
- **Notarisation and stapling** — submit the signed build to Apple's notary service and staple the ticket. This is an automated malware scan, not a human review; no feature restrictions apply.
- **Release packaging and update channel** — DMG or ZIP for download, plus an in-app updater (e.g. Sparkle) since there is no App Store update mechanism. Consider wiring this into the existing GitHub Actions workflow.

#### Phase 2 — split the build

- **Add a second target or build configuration** — e.g. a `DuPane-MAS` target in `project.yml` sharing `Sources/DuPane`, so both editions are generated by `xcodegen` from one manifest.
- **Add a second entitlements file** — keep `Resources/DuPane.entitlements` (sandbox `false`) for direct; add `Resources/DuPane-MAS.entitlements` with `com.apple.security.app-sandbox` true, `com.apple.security.files.user-selected.read-write`, and `com.apple.security.files.bookmarks.app-scope`.
- **Introduce a `MAS_BUILD` compilation condition** — gate the incompatible features below with `#if !MAS_BUILD` rather than deleting them, so the direct edition keeps full functionality from the same source.

#### Phase 3 — MAS compliance

- **Replace the `zip`/`unzip` shell-outs with a native archive implementation** — `PaneView.compressItems`/`uncompressItem` currently spawn `/usr/bin/zip` and `/usr/bin/unzip` through `ProcessRunner`; a sandboxed app cannot spawn them. Commander One's MAS build has full ZIP support, so the feature is achievable — only the mechanism must change. Worth doing for the direct edition too, since shelling out is the weakest part of that code. Keep `ArchiveExtractionSafety`'s zip-slip guard either way. (`Views/PaneView.swift`, `Models/FileOperationService.swift`)
- **Compile out the Command Runner in the MAS build** — `ProcessRunner` spawning `/bin/zsh` has no sandboxed equivalent; this is Commander One's "Command Line"/"Terminal Emulator" restriction. (`Views/CommandRunnerView.swift`, `Models/FileOperationService.swift`)
- **Compile out `.sh` launch via Terminal in the MAS build** — `open -a Terminal` is a spawn of another app with arguments. (`Views/PaneView.swift`, `Views/SidebarView.swift`)
- **Compile out drive eject in the MAS build** — `NSWorkspace.unmountAndEjectDevice` is explicitly on Commander One's MAS restriction list. (`Models/NetworkVolumeMonitor.swift`)
- **Determine empirically what survives in the Network section** — mounting via `NSWorkspace.open("smb://…")`, Bonjour discovery via `NetServiceBrowser`, and mounted-volume enumeration via `statfs` are each unconfirmed under sandbox. Commander One offers FTP/WebDAV in MAS but those are its own clients, not OS-level mounts. Test each, then keep or gate. (`Models/NetworkVolumeMonitor.swift`)
- **Determine whether launching `.app` bundles on double-click survives** — `NSWorkspace.shared.open` on an app bundle; not on Commander One's restriction list, but unconfirmed. (`Views/PaneView.swift`)
- **Add a first-run folder-access grant with persisted security-scoped bookmarks** — present an `NSOpenPanel`, store the chosen root as bookmark data created with `.withSecurityScope`, resolve on launch, and wrap use in `startAccessingSecurityScopedResource()`/`stopAccessingSecurityScopedResource()`. A granted folder extends recursively, so one grant of the home folder or a volume covers most use.
- **Degrade gracefully when access is refused or missing** — every file operation must handle "no permission" as a normal outcome, not an error state, since a person may decline the grant or revoke Full Disk Access later.
- **Optionally detect and prompt for Full Disk Access** — cannot be granted by entitlement; the person must enable it in System Settings > Privacy & Security. Detect absence and explain rather than failing silently.

#### Phase 4 — user guide: one-time approvals (MAS edition)

- **Document the approvals flow in `Docs/UserGuide.html`** — a dedicated section for the MAS edition covering the first-run folder grant (what to choose, and that a folder grant extends to everything inside it), the optional Full Disk Access step with the exact System Settings path, and the fact that both are one-time. Emphasise that these are macOS requirements, not DuPane choices.
- **Add an edition comparison to the user guide and README** — a short table of what the MAS edition cannot do and why, mirroring Commander One's public comparison page, so expectations are set before purchase or download.
- **Point at the guidance from inside the app** — the first-run grant prompt should link to the user-guide section rather than relying on the person finding it.

#### Phase 5 — submission

- **Create the App Store Connect record** — bundle identifier, screenshots, description, category, and the App Privacy questionnaire.
- **MAS signing and provisioning** — App Store distribution certificate and provisioning profile, distinct from the Developer ID identity used in Phase 1.
- **Submit for review and budget for iteration** — unlike notarisation, this is a human review; expect at least one round of feedback.

#### Open questions for Ian

- **Pricing model** — free, paid, or free MAS edition driving direct sales of the fuller edition.
- **Bundle identifier and naming** — whether the MAS edition ships under a separate bundle ID and a distinguishable name, given it is a reduced edition.
